<script lang="ts">
    ...
    let link = $state(path.get('/'))

    const setMode = (mode: string) => {
        site.setMode(mode)
        const [_, __, page, slug] = path.name.split('/')
        if (page === 'about') {
            link = path.get(`/${page}/${slug}`)
        }
        else if (page) {
            link = path.get(`/${page}/${mode}/`)
        }
        else {
            link = path.get('/')
        }
...
{#if isMobile}
    <aside class="flex mobile">
        <a
            ...
            href="{link}"
            class={[ 'btn', 'tooltip', { active: site.mode === 'client' }]}
        >
        ...
        <a
            ...
            href="{link}"
            class={[ 'btn', 'tooltip', { active: site.mode === 'server' }]}
        >
