...
export default class Context {
    ...
    private createPages(): Readable<number[]> {
        return derived([this.rowsPerPage, this.count], ([$rowsPerPage, $count]) => {
            if (!$rowsPerPage || !$count) {
                return [1]
            }
            const pages = Array.from(Array(Math.ceil($count / $rowsPerPage)))
    ...
    private createIsAllSelected(): Readable<boolean> {
        return derived([this.selected, this.rows, this.rows], ([$selected, $rows]) => {
